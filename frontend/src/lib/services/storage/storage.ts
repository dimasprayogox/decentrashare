...
export const storageService = {
	...
	getPublicProfile: async (userId: string) => {
		return apiClient<GetPublicProfileResponse>(`/users/${userId}/public-profile`, {
			method: 'GET'
		});
	},
	...
	searchPublicDocuments: (query: string, limit: number = 12) => {
		return apiClient<SearchPublicDocumentsResponse>(
			`/documents/public/search${buildQueryString({ q: query, limit })}`,
			{
				method: 'GET'
			}
		);
	...
	searchPublicFolders: (query: string, limit: number = 12) => {
		return apiClient<SearchPublicFoldersResponse>(
			`/folders/public/search${buildQueryString({ q: query, limit })}`,
			{
				method: 'GET'
			}
		);
	...
	bulkDownloadDocuments: async (
		documentIds: string[]
	): Promise<{ success: true; fileName: string; documentCount: number }> => {
		return storageService.bulkDownloadItems({ documentIds, folderIds: [] });
	},

	bulkDownloadItems: async ({
		documentIds = [],
		folderIds = []
	}: {
		documentIds?: string[];
	...
	confirmDocumentOnChain: async (documentId: string, txHash: string, blockNumber: number) => {
	...
	restoreDocuments: async (documentIds: string[]) => {
		const response = await apiClient<{ success: boolean; count: number }>('/documents/restore', {
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ documentIds })
		});
	...
	restoreFolders: async (folderIds: string[]) => {
		const response = await apiClient<{ success: boolean; count: number }>('/folders/restore', {
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ folderIds })
		});
	...
	destroyDocuments: async (documentIds: string[]) => {
		const response = await apiClient<{ success: boolean; count: number }>('/documents/destroy', {
			method: 'DELETE',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ documentIds })
		});
	...
	destroyFolders: async (folderIds: string[]) => {
		const response = await apiClient<{ success: boolean; count: number }>('/folders/destroy', {
			method: 'DELETE',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ folderIds })
		});
	...
	updateDocumentsPrivacy: async (
		updates: Array<{ documentId: string; newPrivacy: PrivacyLevel }>
	) => {
	...
	shareFolder: async (
		folderId: string,
		data: { targetUsers: Array<{ userId: string; role: 'VIEWER' | 'EDITOR' }> }
	) => {
	...
	updateDocumentMetadata: async (
		...
		const response = await apiClient<GetDocumentDetailResponse>(`/documents/${documentId}/edit`, {
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json' },
	...
	isFolderNameTaken: (
		...
		return existingFolders.some(
			(folder) => folder.id !== excludeFolderId && folder.name.toLowerCase() === normalized
		);
	}
};
