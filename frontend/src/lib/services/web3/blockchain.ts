...
function validateBatchPayload(payload: BatchBlockchainPayload): void {
	...
	if (Array.isArray(arg1) && Array.isArray(arg2) && Array.isArray(arg3)) {
		...
		if (cids.length !== fileNames.length || cids.length !== fileHashes.length) {
			throw new Error(
				`Array length mismatch: cids=${cids.length}, names=${fileNames.length}, hashes=${fileHashes.length}`
			);
		}
...
export async function recordFilesBatchOnChain(
	...
	const rawAbi = Array.isArray(payload.abi) ? payload.abi : (payload.abi as any)?.abi;
...
export async function tryBatchWithSingleFallback(
	...
	try {
		...
		return {
			...
			blockNumber: firstSuccess
				? (await new ethers.BrowserProvider(window.ethereum).getTransaction(firstSuccess.txHash!))
						?.blockNumber || 0
				: 0,
			success: true,
