...
export function useDelete(onRefresh: () => Promise<void>) {
  ...
  // Internal setters
  function _setDeletingItem(item: DeleteItem | null) { _deletingItem = item; }
  function _setBulkDeleteItems(items: DeleteItem[]) { _bulkDeleteItems = items; }
  function _setIsPermanent(permanent: boolean) { _isPermanent = permanent; }
