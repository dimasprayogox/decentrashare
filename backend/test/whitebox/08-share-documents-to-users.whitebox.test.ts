...
describe('document.service.ts - 08. share/revoke document Whitebox Testing', () => {
  beforeEach(() => {
    resetPrismaMock(prisma);
    Object.values(logger).forEach(fn => fn.mockReset());
  });

  test('Jalur Sukses: owner membagi dokumen ke user lain (direct access granted, privacy -> SPECIFIC_USER)', async () => {
  ...
  test('Jalur Sukses: direct access terakhir di-revoke (document -> PRIVATE)', async () => {
  ...
  test('Jalur Gagal: dokumen tidak ditemukan saat share', async () => {
    const { shareDocumentsToUsers } = await import('../../src/modules/document/document.service?cache-bust=08');
  ...
  test('Jalur Gagal: user bukan owner saat share', async () => {
    const { shareDocumentsToUsers } = await import('../../src/modules/document/document.service?cache-bust=08');
  ...
  test('Jalur Gagal: target user kosong saat share', async () => {
    const { shareDocumentsToUsers } = await import('../../src/modules/document/document.service?cache-bust=08');
    prisma.document.findFirst.mockResolvedValue(documentFactory({ id: 'doc-1', ownerId: 'owner-1' }));
  ...
  test('Jalur Sukses: share dokumen yang aksesnya sudah ada (upsert tetap berjalan)', async () => {
    const { shareDocumentsToUsers } = await import('../../src/modules/document/document.service?cache-bust=08');
    prisma.document.findFirst.mockResolvedValue(documentFactory({ id: 'doc-1', ownerId: 'owner-1' }));
  ...
  test('Jalur Gagal: dokumen tidak ditemukan / unauthorized saat revoke', async () => {
    const { revokeDocumentsAccess } = await import('../../src/modules/document/document.service?cache-bust=08');
    prisma.document.findFirst.mockResolvedValue(null);
  ...
  test('Jalur Sukses: akses tidak ditemukan saat revoke (revokedCount bernilai 0)', async () => {
    const { revokeDocumentsAccess } = await import('../../src/modules/document/document.service?cache-bust=08');
    prisma.document.findFirst.mockResolvedValue(documentFactory({ id: 'doc-1', ownerId: 'owner-1', privacy: 'SPECIFIC_USER' }));
