...
export class UserService {
  ...
  /**
   * Ambil profil lengkap user berdasarkan ID (untuk halaman "me").
   */
  ...
  /**
   * Ambil profil publik user beserta folder dan dokumen publiknya.
   */
  ...
  /**
   * Cari user lain untuk fitur berbagi dokumen/folder.
   */
  ...
  /**
   * Update avatar user: upload ke Pinata lalu update URL di database.
   * Cleanup file temp dan old avatar setelah update (non-blocking).
   */
        ...
        (async () => {
          try {
            ...
            logger.debug(`[Pinata Cleanup] Checking if old avatar should be unpinned`, {
              ...
              shouldUnpin:
                currentUser?.avatarUrl && !currentUser.avatarUrl.includes(oldAvatarHash!),
            });
            ...
            if (currentUser?.avatarUrl && !currentUser.avatarUrl.includes(oldAvatarHash!)) {
              logger.info(
                `[Pinata Cleanup] Attempting to unpin old avatar: ${oldAvatarHash}`,
                { userId }
              );

              const unpinRes = await fetch(
                `https://api.pinata.cloud/pinning/unpin/${oldAvatarHash}`,
                {
                  method: 'DELETE',
                  headers: {
                    Authorization: `Bearer ${process.env.PINATA_JWT}`,
              ...
              logger.debug(
                `[Pinata Cleanup] ⏭️ Skipping unpin - avatar still in use or already changed`,
                { userId, oldAvatarHash, currentAvatarUrl: currentUser?.avatarUrl }
              );
            }
  ...
  /**
   * Update data teks profil user (username, email, bio, website).
   */
  async updateProfile(
    userId: string,
    {
...
// Singleton instance (backward-compatible dengan controller yang pakai userService.method())
